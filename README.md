...
Phase 1 (core language) and all of Phase 2 are complete: IR (2.1), native compilation via Cranelift (2.2), the module system and visibility (2.6), tracing GC + runtime + `defer` syntax (2.3), the benchmark suite + `ListBuilder` / `MapBuilder` transient-mutable accumulators (2.7), the WebAssembly target (2.4 — `wasm32-linear` embed-and-merge plus inline `wasm32-gc`, both byte-identical to native across the full fixture matrix), and JavaScript interop (2.5 — `extern js` as a uniform host-FFI boundary across all five backends). The active work spans two parallel tracks: Phase 3 (tooling — package manager, LSP gap-closing, formatter) and Phase 4 (standard library), which begins with the annotation system (4.5) as its keystone. Async/await with structured concurrency, typed database queries, refinement types, and first-class reactivity for a full-stack web language follow.
