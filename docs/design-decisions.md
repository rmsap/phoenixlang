...
- **Struct fields are intended to be reified at monomorphization time.** The design target is: when a generic struct `Pair<A, B>` is used as `Pair<Int, String>`, the mono pass substitutes each field's `TypeVar` before `struct_layouts` is finalized, so the backend can read concrete field types from the layout and `StructRef` never needs to carry them. **Current reality: this reification is not yet implemented.** `struct_layouts` is keyed by bare name and holds `IrType::TypeVar(T)` for generic-struct fields; monomorphization only specializes function bodies, not struct layouts; `register_method` emits a bare `StructRef(name)` for `self` with no type args. Consequence: generic user-defined structs don't compile in Cranelift today — use-sites hit `IrType::TypeVar` in the backend. See [known-issues.md: *Generic user-defined structs are not supported in compiled mode*](known-issues.md#generic-user-defined-structs-are-not-supported-in-compiled-mode). The "drops args" choice is still the right long-term shape; closing the gap between it and reification is **committed to Phase 2.2**.
...
| `<T: Trait>` method calls in compiled mode | Phase 2.2 follow-up (demand-triggered) | Pre-existing gap (not introduced by dyn work): sema + AST interp support; Cranelift lowers to unresolvable `BuiltinCall`. Fix at IR lowering or in monomorphization. |
| `dyn Trait` over generic user-defined structs | **Phase 2.2 — committed before close** | Lands as a byproduct of the broader ["Generic user-defined structs are not supported in compiled mode"](known-issues.md#generic-user-defined-structs-are-not-supported-in-compiled-mode) fix — same name-keyed tables. Not a deferred follow-up: it ships as part of the in-flight 2.2 work. |
