...
**Operation semantics (all observably identical to native; O(n) where native is O(1) — accepted, consistent with K.7's O(n) lists; fixture maps are tiny):**

- `Op::MapAlloc(pairs)` (literal) — build `$keys`/`$vals` of the literal's length, then insert each pair with **last-wins-on-duplicate, first-position-kept** (scan the keys built so far; if present, overwrite the value in place; else append). Matches native's `from_pairs`. Final `$len` may be < the array length when the literal had duplicate keys (the K.7 `$len < capacity` invariant).
- `Map.get(k) -> Option<V>` — linear scan; `Some(vals[i])` at the first key-equal `i`, else `None`. (Native returns ptr-or-null wrapped to Option; wasm32-gc builds the `Option<V>` enum directly via K.4, with K.4 partial-generic resolution covering the result type.)
- `Map.contains(k) -> Bool`, `Map.length() -> Int` — scan / `$len`.
- `Map.set(k, v) -> Map` / `Map.remove(k) -> Map` — copy-on-write into fresh arrays (immutable API); `set` overwrites in place if present (position kept) else appends; `remove` drops the entry, compacting order.
...
**Decided 2026-06-14 (dyn design lock, confirmed with user). Implemented 2026-06-15 — five of the six `traits_dyn*.phx` fixtures pass five-backend byte-for-byte; the sixth (`dyn` in a *struct field*) is deferred with the broader reference-typed-struct-fields work (see §Scope).**
...
**Deferred: `dyn` as a *struct field* (`traits_dyn_field`).** This fixture stays skipped on wasm32-gc not for any dyn-specific reason but because reference-typed *struct fields* are not lowered there **at all** yet — `wasm_field_type_for` accepts only Int / Float / Bool / String, and a plain `struct Outer { inner: Inner }` is rejected identically (see the integration test `struct_with_nested_struct_field_is_rejected_until_a_later_slice`). Once that slice generalizes struct fields to reference types — reserve/define index pre-assignment for *all* structs (the same machinery this entry added for `$dyn_T`), plus `struct.new`/`struct.get` over ref fields — it unblocks nested-struct / list / map / enum / closure / dyn fields together, and `traits_dyn_field` unskips with them. The `dyn` ABI needs no further work for it; `IrType::DynRef(T)` → `(ref null $dyn_T)` already slots into a field like any other single-slot ref. K.1's deferral note tracks the same gap from the struct side.
