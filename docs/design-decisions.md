...
### Method turbofish (`obj.method<T>(args)`) uses bare angle brackets

**Decided 2026-06-27.** Explicit type arguments on a generic method call are written with bare angle brackets immediately before the argument list — `obj.method<Int>(x)` — matching the constructor form (`Box<Int>(...)`) rather than Rust's `::<>`. The parser commits to a turbofish only when the `<...>` parses as a type-argument list *and* is immediately followed by `(`, so ordinary comparisons (`a.b < c`) and comparison chains (`a.b < c > d`) are left intact for the binary-operator layer.

**Accepted residual ambiguity:** because a parenthesised right operand also supplies the trailing `(`, the expression `a.b < c > (d)` commits as the call `a.b<c>(d)` rather than the comparison `(a.b < c) > (d)`. This is the well-known cost of `::`-less turbofish (C++/TypeScript share it). It is accepted rather than worked around: the construct "compare, then `>` a parenthesised operand" is vanishingly rare, the syntax stays consistent with constructors, and the alternative (`::<>`) adds permanent ceremony to the common case. Pinned by `field_access_comparison_then_paren_commits_as_turbofish` in `phoenix-parser`.
