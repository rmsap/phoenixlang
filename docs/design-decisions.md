...
✅ **Implemented 2026-06-23 (PR 16).** `crates/phoenix-driver/tests/interop_matrix.rs` runs the canonical interop family — `scalars` (scalar round-trip), `strings` (String in + out), `strings_unicode` (multi-byte UTF-8 String round-trip; `byteLen` agrees across backends because UTF-8 bytes are exactly what crosses the wire), `jsvalue` (`JsValue` pass-through), `callbacks` (closures-as-callbacks) — on **all five backends**, each asserting the fixture's `expected.txt` line-for-line (the column comparison normalizes the trailing newline; the always-on Node tier in `interop_node.rs` pins the stricter byte-exact baseline for the WASM columns): the AST and IR interpreters via Rust host stubs registered on the shared `HostRegistry` (one stub set, both interpreters, through a small `HostRegister` trait — the new `phoenix_interp::run_with_host_capture` mirrors the IR interp's entry); native via a linked C host shim overriding the weak defaults (decisions A0/E/G); both WASM targets via the generated glue under Node (the build + `node` invocation shared with the always-on Node tier in `common::interop`). The same program yields identical output through an i32 handle table *and* an `externref`, copy-marshalled *and* scratch-region strings, pinned *and* host-traced closures — A0's mechanism-parity claim made executable. The interp columns need no toolchain and always run, so the matrix never fully skips; the native + WASM columns soft-skip with a visible note under the usual `PHOENIX_REQUIRE_*` gates. **Two glue-tier carve-outs, asserted not silent** (`carve_outs_are_glue_tier_only`): the `dom/*` family (real DOM mutation — browser/jsdom tier, decision I) and `host_effect` (the glue's `ctx.emit` output-ordering channel — the interpreters' `HostContext` exposes only `call_callback`, no `emit`, so a side-effecting host write has no interp/native analogue). The test fails if any *other* `tests/fixtures/interop/` fixture is added without joining all five columns, so the carve-out set can't silently grow.
...
The harness's npm deps (`jsdom`, `playwright-core` — the latter brings no bundled browser) are not committed; a fresh checkout runs `npm ci` in `tests/interop-browser/`. Two new gates keep this from breaking a CI that hasn't provisioned them yet: `PHOENIX_REQUIRE_BROWSER_DEPS=1` (hard-fail if the npm deps are missing) and `PHOENIX_REQUIRE_BROWSER=1` (hard-fail if no browser is launchable); both soft-skip otherwise.

✅ **CI wired 2026-06-23 (PR 17).** A dedicated `interop-browser` job (`.github/workflows/ci.yml`) provisions the deps (`npm ci`) and a matching headless Chromium (`npx playwright-core install --with-deps chromium`, run from the harness dir so npx resolves the *pinned* `playwright-core` — the browser revision can't drift from the launcher) and runs `interop_browser.rs` with all four gates hard (`PHOENIX_REQUIRE_BROWSER_DEPS` / `PHOENIX_REQUIRE_BROWSER` / `PHOENIX_REQUIRE_NODE` / `PHOENIX_REQUIRE_WASM_GC` + `PHOENIX_REQUIRE_RUNTIME_WASM`), so both DOM tiers run on both WASM targets with no silent skip. The fast `check` job stays off the browser path: its `interop_browser` tests soft-skip on the absent npm deps (it sets neither browser gate), while its newly-added `PHOENIX_REQUIRE_WASM_GC=1` makes the *non-DOM* gc interop columns (`interop_node` + `interop_matrix`) hard-required there.
...
- Linear: ✅ **Implemented 2026-06-19.** An `i32` handle into a JS-owned handle table the glue (`wasm/js_glue.rs` `LinearGlue`) manages; Phoenix never dereferences it, only passes it back to externs. The `handles.put` / `handles.get` table mints a handle per host value crossing in and resolves it on the way back out.
- WASM-GC: ✅ **Implemented 2026-06-22.** An `externref` passed directly; the host VM owns and traces it (no handle table). `IrType::JsValue` lowers to `(ref null extern)` (`ValType::EXTERNREF`) in the gc backend's type mapping, and `Op::ExternCall` lowers to a `call` of a custom per-extern import (one import per distinct `(module, name)`, declared before any local function). PR 12 landed scalar + `JsValue` externs; `String` (PR 13) and closures (PR 14) followed; the JS glue (`wasm/wasm_gc/js_glue.rs` `GcGlue`) that satisfies these gc imports and passes real host values as `externref` landed in PR 15.
...
✅ **Implemented** — strings are copied at every binding, never shared: wasm32-linear out via the `(ptr, len)` scratch-copy + in via `phx_string_alloc` into a fresh GC string (2026-06-19, PR 7); wasm32-gc out/in through its linear-memory scratch region via `phx_extern_str_to_scratch` / `phx_extern_str_from_scratch` (2026-06-22, PR 13); native through the `(ptr, len)` fat pointer + `phx_string_alloc` (2026-06-20, PR 9); the interpreters hand the host an owned `String`, already a copy. No backend aliases the other side's bytes — pinned end-to-end by the `strings` + `strings_unicode` columns of the five-backend matrix (a multi-byte UTF-8 string round-trips identically through all five).
