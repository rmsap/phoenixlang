...
These are what make Phoenix worth choosing over a TypeScript (or Go) web stack. Implement them after the core is solid.
...
- [x] **Python enforced only the extractable (numeric/length) `where` subset, silently accepting violations Go/TS reject.** **✅ Closed 2026-06-23.** The Python target mapped a field's `where` constraint to pydantic `Field(...)` / FastAPI `Form(...)` validation kwargs (`ge`/`le`/`gt`/`lt`/`min_length`/`max_length`), which can only express comparisons against `self`/`self.length`. A non-reducible constraint — e.g. `email: String where self.contains("@")` — produced **no** kwarg, so a Python server **silently accepted** input Go and TS (which translate the full expression) reject — a cross-language enforcement divergence that no same-language round-trip could catch. Closed by translating the FULL constraint expression to Python (`constraint_expr_to_python`, a precedence-aware emitter mirroring `constraint_expr_to_go`) and enforcing it: JSON bodies/projections via a `@model_validator(mode="after")` (`emit_constraints_validator`), multipart bodies via an inline route check (both raise 422). `Field`/`Form` now carry only the wire `alias=`. The emitter is pre-wrapped/`not in`-aware so the output stays `black --check`- and `ruff`-clean (UP034/SIM102/E713/E501) without shelling out to a formatter. Regression: `python_tests.rs` (`model_with_constraints`, `float_constraints`, `body_partial`, `camel_case_struct_field_aliases_to_wire_name`, and the three `*multipart_scalar_where_constraint*` tests now assert the inline `if not (…)` / `not in` enforcement), the four re-baselined model snapshots, and the existing constraint round-trip cases (which exercise the new validator path at runtime, returning 422). Rationale: [phoenix-gen-design-decisions.md](../phoenix-gen-design-decisions.md#phoenix-gen--python-constraint-expression-parity-closed-2026-06-23).
- [x] **Generated names can collide — types with user-defined types, endpoints with each other.** **✅ Closed 2026-06-10.** The endpoint-derived generated type names — the envelopes `<Endpoint>Result` / `<Endpoint>Page` / `<Endpoint>Response` plus the request-body types `<Endpoint>Body` and `<Endpoint>ClientBody` (Go, multipart) and the fixed-name multipart helper `FileUpload` (Go) — were not reserved: a user struct of the same name, or for `ClientBody` an endpoint pair like `upload` / `uploadClient` whose names differ by a `Client` infix, produced two declarations of one type in the generated output, a compile error in Go/TS but a **silent class redefinition** in Python (and for `Body`, a silent first-wins dedupe in every backend). Separately, two endpoints differing only in first-letter case (`getUser` / `GetUser`) collided on the generated Go **method and handler names** (built with `capitalize(name)`) even when neither generated a single type. Closed with two sema checks (`check_exported_name_collision` and `check_generated_type_collisions` in `check_endpoint.rs`): the exported-name check rejects case-colliding endpoint pairs outright (subsuming their same-stem type collisions), and the type check rejects user-type and cross-stem claimants — both built on the shared `phoenix_common::idents::capitalize` so the predicted names cannot drift from codegen. Regression tests: the `envelope_collision_*`, `body_collision_*`, `exported_name_*`, and `file_upload_*` suites plus `distinct_endpoint_envelopes_do_not_collide`, `duplicate_endpoint_name_does_not_also_report_envelope_collision`, `case_colliding_endpoints_report_one_collision`, and `collision_with_user_type_declared_after_endpoint_rejected` in `crates/phoenix-sema/src/check_endpoint.rs`; reverting either check resurfaces a silent or unexplained generated-code break. Rationale: [phoenix-gen-design-decisions.md](../phoenix-gen-design-decisions.md#generated-type-name-collision-check).
