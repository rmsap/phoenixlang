...
**Status: in progress.** Landed so far: explicit `json.decode<T>` type arguments; `json.encode` for scalars, non-generic structs, `Option`, non-generic enums, `List<T>`, and `Map<String, V>`; `json.decode` for scalars, `Option<T>`, `List<T>`, `Map<String, V>`, and non-generic structs and enums (adjacently tagged; nested, imported, and cyclic; an absent `Option` struct field decodes to `None` — see [design-decisions §Phase 4.6 B](../design-decisions.md#b-an-absent-option-struct-field-decodes-to-none-absent--null)), with cross-module field privacy enforced on both operations. This completes the decodable-type surface bar non-`String`-key maps (a deferred follow-up). Remaining: contextual inference of `T` (see known-issues), `@jsonName`/`@skip`, and the wasm32-gc DOM port. First consumer of the [Annotation system (4.5)](#45-annotation-system), which has landed. **Depends on:** Annotations (4.5, complete) for `@jsonName`/`@skip`; generic monomorphization (2.2, complete). Independent of all Phase 3 work (see [Parallel-track note](#46-parallel-track-note) below).
...
- **`JsonError`** is a pre-registered builtin enum (like `Option`/`Result`): minimally `ParseError(String)`, `TypeMismatch(String)`, `MissingField(String)`. Decode never panics on bad input — it returns `Err`. The type name `JsonError` is reserved (user redeclaration as enum/struct/function is rejected), but the **variant names are not** — a user enum may still declare its own `ParseError`/`TypeMismatch`/`MissingField`. The consequence is a deliberate namespace widening: a bare-name use of one of those variants that was previously unambiguous becomes ambiguous between the user enum and the builtin and is diagnosed (the fix is to rename the colliding user variant — variant resolution is by bare name only, so there is no construction-site type qualification to fall back on). This only affects programs that already reused one of the three names unqualified.
