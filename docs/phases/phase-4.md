...
**Status: in progress.** Landed so far: explicit `json.decode<T>` type arguments; `json.encode` for scalars, non-generic structs, `Option`, non-generic enums, `List<T>`, and `Map<String, V>`; `json.decode` for scalars, `Option<T>`, `List<T>`, and non-generic structs and enums (adjacently tagged; nested, imported, and cyclic; an absent `Option` struct field decodes to `None` — see [design-decisions §Phase 4.6 B](../design-decisions.md#b-an-absent-option-struct-field-decodes-to-none-absent--null)), with cross-module field privacy enforced on both operations. Remaining: decode of `Map`, contextual inference of `T` (see known-issues), `@jsonName`/`@skip`, and the wasm32-gc DOM port. First consumer of the [Annotation system (4.5)](#45-annotation-system), which has landed. **Depends on:** Annotations (4.5, complete) for `@jsonName`/`@skip`; generic monomorphization (2.2, complete). Independent of all Phase 3 work (see [Parallel-track note](#46-parallel-track-note) below).
