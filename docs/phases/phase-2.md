...
- **[Generic user-defined structs are not supported in compiled mode](../known-issues.md#generic-user-defined-structs-are-not-supported-in-compiled-mode)** — `struct Container<T>` works under `phoenix run` but panics under `phoenix build`. `struct_layouts` is keyed by bare name and holds `IrType::TypeVar(T)`; `register_method` emits a bare `StructRef(name)` for `self`; monomorphization only specializes function bodies. Fix spans those three sites plus `IrModule::dyn_vtables` / `method_index` keying so `dyn Trait` over generic structs works as a byproduct. Currently gated by a `debug_assert!` tripwire in `register_method`'s struct branch. **Must close before 2.2 wraps** — keeps the `phoenix run` / `phoenix build` semantic gap from shipping to users, and lets the monomorphization design settle before more code assumes the current name-keyed tables.
