...
2. **PR 2** — WASM emission scaffolding (via `wasm-encoder`) + WASI host imports. New `phoenix-cranelift/src/wasm/` module tree implements `Target::Wasm32Linear`'s codegen path; `phoenix build --target wasm32-linear hello.phx` produces a `.wasm` that runs under wasmtime and prints. Translator surface is minimal — constants, `BuiltinCall(@print)` for `i64` / `bool` / `String`, function calls, void return — enough for hello-world. The rest of the IR-op coverage lands in PR 3 alongside the GC port. Integration tests in `crates/phoenix-cranelift/tests/compile_wasm_linear.rs` always do structural validation (`wasmparser::validate`) and additionally execute under `wasmtime` when it's on `$PATH`; CI sets **`PHOENIX_REQUIRE_WASMTIME=1`** to turn the execute-tier skip into a hard failure (same gating shape as the §2.3 `PHOENIX_REQUIRE_VALGRIND` gate).
3. **PR 3** — Linear-memory `MarkSweepHeap` port (dlmalloc, single-threaded TLS adaptation, `phx_gc_shutdown` via `proc_exit`). Lifting `INeg` (on the critical path for the GC port's IR coverage) also unblocks two `phx_print_i64` test follow-ups that PR 2 couldn't reach from any source-level fixture; the deferred tests and their rationale are listed inline in [`compile_wasm_linear.rs`](../../crates/phoenix-cranelift/tests/compile_wasm_linear.rs)'s negative-integer comment block.
