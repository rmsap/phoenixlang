...
- [ ] **Allocation throughput bench landed.** `crates/phoenix-bench/benches/allocation.rs` exists, runs cleanly under `cargo bench -p phoenix-bench --bench allocation`, and produces numbers for the four size buckets × two tag modes (`Unknown` vs `String`). The first run's numbers are committed as the Phase-2.3-end baseline per the storage decision above.
