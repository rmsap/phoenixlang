...
- [ ] **No `known-issues.md` entry targeted at Phase 2.3.** The map-lookup entry was removed in commit `d34fb56` (hash-map fix); the sortBy entry is removed in this change. Full closure descriptions live in the corresponding §2.3 bug-closure bullets above. The "Memory leaks (no GC yet)" entry is functionally closed but stays in known-issues.md until the valgrind gate below ticks.
