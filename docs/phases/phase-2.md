...
- [ ] **All Phase-2.6 bug-closure entries have regression tests.** Each entry under "Bugs closed in this phase" maps to a topic-specific test that fails when the fix is reverted. Closure-capture ambiguity is **✅ Closed 2026-04-27**: `tests/fixtures/closures_ambiguous_captures.phx` (registered in the three-backend matrix as `matrix_closures_ambiguous_captures`) compiles and runs identically under `phoenix run`, `phoenix run-ir`, and `phoenix build`; reverting the env-pointer ABI fix would resurface the original "ambiguous indirect call" `CompileError` in compiled mode. Default-expression visibility is **✅ Closed 2026-04-30**: the six wrapper-synthesis tests in `crates/phoenix-ir/src/tests.rs` (listed on the bug-closure entry above) cover the synthesis pass, the chained-wrapper ordering invariant, the method-default form, the closure-valued-default corner, and a multi-module privacy sweep that asserts no IR function outside the wrapper directly calls the private symbol; reverting the wrapper-synthesis pass would resurface the inline-default path and trip the multi-module test. The exit-criteria multi-file fixture (module A defines `public function f(x: Int = privateHelper())` and module B imports `f`) becomes available once cross-module call sites are exercisable end-to-end and lands as a `tests/fixtures/multi/` entry under the Three-backend matrix criterion below.
