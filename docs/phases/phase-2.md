...
Four codebase-hygiene refactors land alongside the module-system work. The first two are paid for by the module-system scope itself (multi-file diagnostics, evolving parser AST) and must be complete before Phase 3.2 (LSP). The remaining two are IR-shape refactors that pair with the `Value::Closure` rework because all three touch the same surface (`IrFunction`, `IrModule`, `ValueId` allocation) — bundling them amortizes the disruption rather than ripping the IR open twice. See [design-decisions.md](../design-decisions.md) and [known-issues.md](../known-issues.md):
...
- **[Generic-template stubs typed split](../known-issues.md#generic-template-stubs-tracked-by-a-bool-flag)** — replace `IrFunction.is_generic_template: bool` with a typed split (a `ConcreteFunctions` newtype iterator, or separate `functions` / `templates` fields) so the filter is enforceable at the type-system level rather than at every call site. The 2026-04-20 audit caught two slips (`IrModule::Display`, `ir_analysis.rs`) that had bypassed the bool filter; pairs with `Value::Closure` because both are `IrModule` / `IrFunction` shape changes.
- **[`ValueId` allocator typed split](../known-issues.md#irfunctionvalue_types-is-a-parallel-index-without-a-type-level-guarantee)** — introduce a `ValueIdAllocator` newtype that owns both the counter and the parallel `value_types` index, so allocating a `ValueId` and assigning its type are the same operation. Length-mismatch bugs become compile errors instead of runtime verifier errors. Same shape change family as the typed-split above.
...
- [ ] **All Phase-2.6 design decisions implemented.** All four `**[…]**` bullets in §2.6's "Refactors bundled into this phase" carry a `✅ Implemented YYYY-MM-DD` marker.  Concretely: (a) the diagnostic-builder *foundation* is in (2026-04-27), so this box closes when module-system diagnostics actually exercise `with_note` for at least one cross-file error path — see the dedicated criterion below; (b) `Value::Closure` no longer stores `phoenix_parser::ast::Block` — `rg 'Closure.*ast::Block\|Closure.*parser::ast' crates/phoenix-interp/src/` returns nothing, and the interpreter walks IR for closure bodies the same way it walks IR for top-level functions; (c) `IrFunction.is_generic_template: bool` is gone — `rg 'is_generic_template' crates/` returns nothing, and template / concrete iteration goes through the typed split; (d) `ValueId` allocation and `value_types` assignment are a single operation — no public API for "allocate a `ValueId` without assigning a type", and the verifier's `verify_value_types_index` length-mismatch check becomes structurally unreachable.
- [ ] **All Phase-2.6 bug-closure entries have regression tests.** Each entry under "Bugs bundled into this phase" maps to a topic-specific test that fails when the fix is reverted: closure-capture ambiguity (two closures with identical user signatures but different captures, passed through a block parameter, must run identically across all three backends — exactly the pattern that forced the simplification of `tests/fixtures/closures.phx`); default-expression visibility (a multi-file fixture where module A defines `public function f(x: Int = privateHelper())` and module B imports `f` — calling `f()` from B must work, and `privateHelper` must remain unimportable from B).
