...
- [x] **Workspace test/clippy/fmt clean on the 2.2 branch.** `cargo test --workspace` (2,573 baseline tests + 6 new id-alignment / stability tests) green; `cargo clippy --workspace --tests` zero warnings; `cargo fmt --all -- --check` clean.
- [ ] **Three-backend roundtrip matrix on every `tests/fixtures/*.phx`.** Today: `crates/phoenix-driver/tests/run_ir_cli.rs` exercises `run` vs `run-ir` for a handful of fixtures, and per-feature compile tests under `crates/phoenix-cranelift/tests/compile_*.rs` cover topical surface; what's missing is a single test driver that walks every `tests/fixtures/*.phx`, runs each under all three backends (`phoenix run`, `phoenix run-ir`, `phoenix build`), and asserts byte-identical stdout.  Adding it is small but it's the canonical "are the backends still in agreement" gate.
- [ ] **No `unreachable!()` reachable from a well-typed program in the Cranelift backend.** Audit `unreachable!()` / `unreachable!(…)` sites in `crates/phoenix-cranelift/src/translate/` (currently ~14 across `arith.rs`, `calls.rs`, `data.rs`, `list_methods.rs`, `map_methods.rs`, `mutable.rs`).  Each site needs to be either (a) eliminated, (b) replaced with a proper diagnostic-emitting panic, or (c) accompanied by a comment + test demonstrating that sema rejects every program that could reach it.  The "<T: Trait> → dyn Trait coercion" bug closed above was an instance of this class — there are 14 more potential ones.
- [ ] **`phoenix-ir` verifier has one negative test per invariant.** Today only `crates/phoenix-ir/src/verify/dyn_ops.rs` carries hand-written-IR negative tests.  Other verifier rules (block-terminator presence, value-type compatibility, function-signature agreement, etc.) need parallel coverage so the verifier's claims are themselves testable.

When every box above is ticked, Phase 2.2 closes and Phase 2.3 (GC + runtime) becomes the active phase.
