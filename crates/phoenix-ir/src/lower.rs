...
pub fn lower_type(ty: &Type, check_result: &ResolvedModule) -> IrType {
    match ty {
        ...
        // `DateTime` is a Phoenix Gen scalar with no language-level literals or
        // operations yet; at runtime it is an RFC 3339 string, so it shares
        // `String`'s representation (branded-string semantics). Unlike `File`,
        ...
        // backend binding yet (the interpreters bind it in PR 4, native in PR 9,
        // the WASM backends in PRs 5–8 / 12–15), so a program that actually
        // *materializes* a `JsValue` still can't execute end-to-end; each
        // backend returns a clean "not supported yet" error on `ExternCall`. The
        // type itself, however, lowers and round-trips through the IR cleanly.
        Type::JsValue => IrType::JsValue,
