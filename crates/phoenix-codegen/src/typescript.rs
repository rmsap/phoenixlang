...
impl<'a> TsGenerator<'a> {
    ...
    fn emit_fastify_route(&mut self, ep: &EndpointInfo) {
        ...
        let args = emit_route_prelude(&mut body, ep, FASTIFY_DIALECT, si, ni);
    ...
    fn emit_server_route(&mut self, ep: &EndpointInfo) {
        ...
        // the accessor vocabulary differs — see [`emit_route_prelude`].
        let args = emit_route_prelude(&mut body, ep, EXPRESS_DIALECT, si, ni);
...
fn emit_route_prelude(
    ...
    if let Some(ref ep_body) = ep.body {
        ...
        if ep.body_is_multipart {
            ...
            // No constraints to validate, but the request body is untyped
            // (`any`/`unknown`); cast to the body type so the handler call is
            // type-safe (and strict `no-unsafe-*` lint rules are satisfied).
