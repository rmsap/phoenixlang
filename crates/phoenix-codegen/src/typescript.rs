...
impl<'a> TsGenerator<'a> {
    ...
    fn generate(mut self, program: &Program) -> GeneratedFiles {
        ...
        // Close the `api` object literal (assigned to a const → needs `;`). For an
        // endpoint-less schema the preamble already emitted the whole empty `{}`.
        ...
        // A schema with no endpoints (a types-only package) yields an empty handler
        // interface, which trips eslint's `no-empty-object-type`. Emit a
        // `Record<string, never>` alias (an object with no properties) instead.
        if self.check_result.endpoints.is_empty() {
            self.handlers_out
                .push_str("export type Handlers = Record<string, never>;\n");
        } else {
    ...
    fn emit_client_preamble(&mut self) {
        ...
        self.client_out.push_str("}\n\n");
        // An endpoint-less schema yields an empty object — emit it whole (`{}`),
        // since prettier collapses `{\n}` to `{}` (the close below is then skipped).
        if self.check_result.endpoints.is_empty() {
    ...
    fn emit_server_imports(&mut self) {
        ...
        // `Request`/`Response` are referenced only by route handlers, and `Handlers`
        // only by the `createRouter` param — a schema with no endpoints has neither,
        // so importing them would be unused.
        if !self.check_result.endpoints.is_empty() {
            self.server_out
    ...
    fn emit_fastify_imports(&mut self) {
        ...
        // `Handlers` is referenced only by the `createRouter` param, which a
        // no-endpoint schema omits.
        if !self.check_result.endpoints.is_empty() {
            self.server_out
                .push_str("import type { Handlers } from \"./handlers\";\n");
        }
    ...
    fn emit_fastify_router(&mut self) {
        // With no endpoints, `handlers` is omitted (an unused param can't pass
        // no-unused-vars; `Handlers` is then unimported, gated above) and the
        // plugin's `app` is `_`-prefixed — `_app` sits before the used `done`, so
        // no-unused-vars' "after-used" mode leaves it alone, exactly like `_opts`.
        let no_ep = self.check_result.endpoints.is_empty();
        let params = if no_ep { "" } else { "handlers: Handlers" };
        let app = if no_ep { "_app" } else { "app" };
        self.server_out.push_str(&format!(
            "export function createRouter({params}): FastifyPluginCallback {{\n"
        ));
        self.server_out
            .push_str(&format!("  return ({app}, _opts, done) => {{\n"));
    ...
    fn emit_server_router(&mut self) {
        // With no endpoints no route references `handlers`, so omit the param
        // entirely (a single unused param can't dodge no-unused-vars; `_handlers`
        // is still flagged). `Handlers` is then unimported (gated above).
        let params = if self.check_result.endpoints.is_empty() {
            String::new()
        } else {
            "handlers: Handlers".to_string()
        };
        self.server_out.push_str(&format!(
