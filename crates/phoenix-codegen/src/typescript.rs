...
impl<'a> TsGenerator<'a> {
    ...
    fn generate(mut self, program: &Program) -> GeneratedFiles {
        ...
        // before the types/revivers that reference them. Emitted in the
        // `ts_branded_scalars` order for stable output.
        for (target, _, _) in ts_branded_scalars() {
            if self.schema_uses_scalar(program, &target) {
...
fn ts_type_needs_revival(ty: &Type, set: &BTreeSet<String>) -> bool {
    match ty {
        ...
        // (validate+brand). All are post-decode transforms the revival pass does.
        Type::DateTime | Type::Uuid | Type::Decimal => true,
