...
impl<'a> TsGenerator<'a> {
    ...
    fn generate(mut self, program: &Program) -> GeneratedFiles {
        ...
        // The branded `Uuid` alias + `parseUuid` validator, before the types/
        // revivers that reference them.
        if self.schema_uses_uuid(program) {
            self.emit_uuid_helpers();
        }
    ...
    /// Whether the schema references `Uuid` anywhere a generated TS file would
    /// name the branded alias or call `parseUuid`: struct fields, body fields,
    /// query/request-header/response-header params, the response type, or a
    /// pagination item type. Gates the one-time `Uuid`/`parseUuid` emission.
    fn schema_uses_uuid(&self, program: &Program) -> bool {
        let in_struct = program.declarations.iter().any(|d| {
            matches!(d, Declaration::Struct(s)
                if self
                    ...
                    .is_some_and(|si| si.fields.iter().any(|f| type_mentions_uuid(&f.ty))))
        });
        let in_ep = self.check_result.endpoints.iter().any(|ep| {
            ep.query_params.iter().any(|q| type_mentions_uuid(&q.ty))
                || ep.headers.iter().any(|h| type_mentions_uuid(&h.ty))
                || ep
                    .response_headers
                    .iter()
                    .any(|h| type_mentions_uuid(&h.ty))
                || ep.response.as_ref().is_some_and(type_mentions_uuid)
                || ep
                    .body
                    .as_ref()
                    .is_some_and(|b| b.fields.iter().any(|f| type_mentions_uuid(&f.ty)))
                || ep
                    .pagination
                    .as_ref()
                    .is_some_and(|p| type_mentions_uuid(&p.item_type))
        });
        in_struct || in_ep
    }

    /// Emits the branded `Uuid` alias and the `parseUuid` validator into types.ts
    /// (once, when the schema uses `Uuid`). `parseUuid` checks the RFC 4122 format
    /// and brands the string; the decode path runs it on every `Uuid` value read
    /// off the wire (response bodies/headers client-side, request bodies/params
    /// server-side). No native JS UUID type exists to revive into, so the brand +
    /// runtime check is how a decoded `Uuid` differs from a bare string.
    fn emit_uuid_helpers(&mut self) {
        self.types_out.push_str(
            "/** A UUID string (RFC 4122), branded so a bare `string` cannot be used as\n \
             * one without going through `parseUuid`. */\n\
             export type Uuid = string & { readonly __uuidBrand: unique symbol };\n\n\
             const UUID_RE =\n  \
             /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;\n\n\
             /** Validates the RFC 4122 format and brands the value as `Uuid`, throwing\n \
             * otherwise. Run by the generated decode path on each wire `Uuid`. */\n\
    ...
    fn emit_client_imports(&mut self) {
        ...
        // `Uuid` is a builtin that, unlike other scalars, needs its alias imported
        // wherever a signature names it (query/request-header params, or a bare
        // `Uuid`/`Uuid[]`/… response/page-item return type). `parseUuid` (a value)
        // is needed wherever the decode path calls it directly: a bare-`Uuid`
        // payload leaf, or a `Uuid` response header. (Struct payloads call their
        // reviver, which calls `parseUuid` inside types.ts — no client import.)
        for ep in &self.check_result.endpoints {
            let names_uuid = ep.query_params.iter().any(|q| type_mentions_uuid(&q.ty))
                || ep.headers.iter().any(|h| type_mentions_uuid(&h.ty));
            let payload = if let Some(ref pag) = ep.pagination {
                Some(&pag.item_type)
            ...
            let payload_names_uuid = payload.is_some_and(type_mentions_uuid);
            if names_uuid || payload_names_uuid {
                type_imports.insert("Uuid".to_string());
            }
            if payload.is_some_and(leaf_is_uuid)
                || ep
                    .response_headers
                    .iter()
                    .any(|h| type_mentions_uuid(&h.ty))
            {
                reviver_imports.insert("parseUuid".to_string());
            }
        }
    ...
    fn emit_handler_imports(&mut self) {
        ...
        for ep in &self.check_result.endpoints {
            ...
            // `Uuid` is a builtin the handler signature still names (query/header
            // params, or a bare non-envelope `Uuid` response) — collect it like a
            // type import, since `collect_import_names` only picks up `Named`s.
            let bare_response_uuid = !ep.response_is_binary
                && ep.response_headers.is_empty()
                && ep.pagination.is_none()
                && ep.response_statuses.is_empty()
                && ep.response.as_ref().is_some_and(type_mentions_uuid);
            if ep.query_params.iter().any(|q| type_mentions_uuid(&q.ty))
                || ep.headers.iter().any(|h| type_mentions_uuid(&h.ty))
                || bare_response_uuid
            {
    ...
    fn emit_server_shared_type_imports(&mut self) {
        ...
        // `parseUuid` (a value): the server validates+brands a `Uuid` query or
        // request-header param through it. (Body `Uuid`s go via the body reviver
        // imported above, which calls `parseUuid` inside types.ts.)
        if self.check_result.endpoints.iter().any(|ep| {
            ep.query_params.iter().any(|q| type_mentions_uuid(&q.ty))
                || ep.headers.iter().any(|h| type_mentions_uuid(&h.ty))
        }) {
            value_imports.push("parseUuid".to_string());
        }
        if !value_imports.is_empty() {
...
fn query_param_coercion_with(
    ...
    let coerced = match &qp.ty {
        ...
        // `parseUuid` validates + brands; like `new Date` it needs a `string`, and
        // `raw` here is `string | undefined`, so the `as string` cast is required.
        Type::Uuid => format!("parseUuid({raw} as string)"),
        Type::Generic(name, args) if name == "Option" && !args.is_empty() => {
            ...
            match &args[0] {
                ...
                Type::Uuid if raw_is_str_opt => {
                    format!("{raw} !== undefined ? parseUuid({raw}) : undefined")
                }
                Type::Uuid => {
                    format!("{raw} !== undefined ? parseUuid({raw} as string) : undefined")
                }
...
/// Whether `ty` is (or has a generic arg that is) a `Uuid`. Does NOT recurse
/// through `Named` structs — callers scan struct fields directly. Used to decide
/// the `Uuid` alias / `parseUuid` imports a file needs.
fn type_mentions_uuid(ty: &Type) -> bool {
    match ty {
        Type::Uuid => true,
        Type::Generic(_, args) => args.iter().any(type_mentions_uuid),
        _ => false,
    }
}

/// Whether peeling `Option`/`List`/`Map` reaches a bare `Uuid` value leaf — i.e.
/// the decode path calls `parseUuid` directly (not via a struct reviver), so the
/// file referencing it must import `parseUuid`.
fn leaf_is_uuid(ty: &Type) -> bool {
    match ty {
        Type::Uuid => true,
        Type::Generic(n, args) if n == "Option" && args.len() == 1 => leaf_is_uuid(&args[0]),
        Type::Generic(n, args) if n == "List" && args.len() == 1 => leaf_is_uuid(&args[0]),
        Type::Generic(n, args) if n == "Map" && args.len() == 2 => leaf_is_uuid(&args[1]),
        _ => false,
    }
}
...
fn ts_type_needs_revival(ty: &Type, set: &BTreeSet<String>) -> bool {
    match ty {
        // `DateTime` → `new Date(...)`; `Uuid` → `parseUuid(...)` (validate+brand).
        // Both are post-decode transforms the revival pass performs.
        Type::DateTime | Type::Uuid => true,
...
fn ts_revive_expr(ty: &Type, expr: &str, set: &BTreeSet<String>) -> Option<String> {
    match ty {
        ...
        // `parseUuid` validates the RFC 4122 format and brands the string.
        Type::Uuid => Some(format!("parseUuid({expr})")),
...
fn type_to_ts(ty: &Type) -> String {
    match ty {
        ...
        // A `Uuid` is a branded `string` alias (`type Uuid = string & {…}`):
        // distinct from a bare `string` at compile time, but a string at runtime.
        // The decode pass validates+brands via `parseUuid` (no native JS UUID type
        // to revive into). See `docs/design-decisions.md`.
...
fn response_header_coercion(h: &phoenix_sema::checker::HeaderParamInfo) -> String {
    ...
    if is_option {
        match inner {
            ...
            Type::Uuid => {
                format!("{raw} !== null ? parseUuid({raw} as string) : undefined")
            }
