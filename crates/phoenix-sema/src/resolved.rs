...
pub struct ResolvedModule {
    ...
    /// Static argument type at each `json.encode(value)` call site, keyed
    /// by the `MethodCallExpr`'s source span. The IR JSON-encode
    /// synthesis pass reads this to collect the types needing encoders and
    /// to dispatch each call site to the right synthesized encoder.
    ///
