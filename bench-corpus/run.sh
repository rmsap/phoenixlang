...

{
  ...
  echo
  # All inline backticks inside this `{ ... } > $OUT_PATH` render
  # block must be backslash-escaped. Within double quotes bash
  # treats `…` as command substitution and tries to execute the
  # quoted text — `docs/perf-baselines/` got interpreted as a
  # command in an earlier draft and aborted the render after the
  # bench loop. Single quotes would dodge the issue but break
  # variable expansion (we need ${RUNS}, ${WARMUP}, etc).
